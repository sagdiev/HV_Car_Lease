...
for j in range(COUNT_EXPERIMENTS):
    ...
    for p in range(len(COUNT_SELLS_BY_MONTHS)):  # Перебор всех месяцев продаж
        ...
        for i in range(sells_in_period):  # Перебор всех продаж в конкретный месяц
            ...
            seed_i_list                = random_choice_seed_list(4, seed_i)
            seed_client_type           = seed_i_list[0]
            seed_cost_flat             = seed_i_list[1]
            ...
            if id_user > max_id_fixed_user:
                ...
                client_type          = df_start_units[column_client_type][id_user]
                client_type_pair     = CLIENT_TYPE_BY_SAVING_PAIR[CLIENT_TYPE_BY_SAVING_SIGN.index(client_type)]
            ...
            period_saving_start             = p
            total_amount_saving             = round(cost_flat * PERCENT_SAVING / 100, 2)
            amount_saving_monthly           = round(total_amount_saving / period_saving, 2)
            percent_postpayments            = PERCENT_POSTPAYMENTS[PERIOD_POSTPAYMENTS.index(period_postpayments)]
            total_amount_postpayments_base  = cost_flat - total_amount_saving
            ...
            client_status                   = CLIENT_STATUS_BEFORE_START

            df.loc[k, column_id_user]                           = id_user
            df.loc[k, column_client_type]                       = client_type
            df.loc[k, column_cost_vehicle]                         = cost_flat
            df.loc[k, column_period_saving]                     = period_saving
            df.loc[k, column_period_saving_start]               = period_saving_start
            df.loc[k, column_amount_saving_monthly]             = amount_saving_monthly
            df.loc[k, column_total_amount_saving]               = total_amount_saving
            df.loc[k, column_accum_amount_saving]               = 0
            df.loc[k, column_accum_amount_saving_spent]         = 0
            df.loc[k, column_prewaiting_period]                 = 0
            df.loc[k, column_waiting_period]                    = 0
            df.loc[k, column_waiting_max_status]                = 0
            df.loc[k, column_flat_received]                     = 0
            df.loc[k, column_period_postpayments]               = period_postpayments
            df.loc[k, column_percent_postpayments]              = percent_postpayments
            df.loc[k, column_total_amount_postpayments_base]    = total_amount_postpayments_base
            df.loc[k, column_amount_postpayment_monthly]        = amount_postpayment_monthly
            df.loc[k, column_total_amount_postpayments]         = total_amount_postpayments
            df.loc[k, column_client_status]                     = client_status
            df.loc[k, column_rank]                              = 0
            df.loc[k, column_postpayment_percent_amount_list]   = postpayment_percent_amount_list
            df.loc[k, column_credit_base_cover_list]            = credit_base_cover_list
            df.loc[k, column_credit_base_list]                  = credit_base_list


            id_user += 1
            k += 1
            print('exp', j+1, 'unit', k, 'param', client_status, client_type, cost_flat,
                  '| save', period_saving, 'm', period_saving_start, amount_saving_monthly,
                  '| postpay', period_postpayments, 'm', percent_postpayments, '%', amount_postpayment_monthly)
    ...
    for t in range(COUNT_MONTHS_OF_CALC):
        ...
        amount_saving_t                     = 0
        amount_postpayments_t               = 0
        amount_postpayment_percent_t        = 0
        amount_postpayment_base_cover_t     = 0

        income_onboard_payment_t            = 0
        income_buy_payment_t                = 0
        income_discount_t                   = 0

        cost_buy_payment_realtor_t          = 0
        cost_buy_payment_notary_t           = 0
        cost_buy_payment_tax_t              = 0
        cost_buy_payment_bank_t             = 0
